...
export const quizzes: Quiz[] = [
  ...
  },
  {
    ...
    questions: [
      ...
      {
        id: 36,
        text: 'What is a "sequence point" in C?',
        type: 'mcq',
        options: ['A point in the program where a loop begins or ends.', 'A point in program execution at which all side effects of previous evaluations are complete.', 'A debug breakpoint.', 'The end of a function.'],
        correctAnswer: 'A point in program execution at which all side effects of previous evaluations are complete.',
        explanation: 'Sequence points (like the semicolon `;` or the comma `,` operator) prevent undefined behavior from expressions like `i = i++`, where a variable is modified more than once between sequence points.',
      },
      {
        id: 37,
        text: 'What is the "strict aliasing rule"?',
        type: 'mcq',
        options: ['A rule that all pointers must have unique names.', 'A rule that you can only have one pointer to any given object.', 'A rule that allows a compiler to assume that pointers of different types do not point to the same memory location.', 'A rule about function pointer syntax.'],
        correctAnswer: 'A rule that allows a compiler to assume that pointers of different types do not point to the same memory location.',
        explanation: 'This rule allows for significant optimizations. Violating it, for example by casting an `int *` to a `float *` to read an `int`\'s memory, is undefined behavior (with some exceptions like `char *`).',
      },
      {
        id: 38,
        text: 'What is the result of `i = i++` in C?',
        type: 'mcq',
        options: ['The value of `i` is incremented by 1.', 'It is guaranteed to be the original value of `i`.', 'It results in undefined behavior.', 'It is a compile-time error.'],
        correctAnswer: 'It results in undefined behavior.',
        explanation: 'The variable `i` is being modified twice between sequence points (once by `i++` and once by the assignment `=`), which the C standard explicitly defines as undefined behavior.',
      },
      {
        id: 39,
        text: 'What is "Duff\'s Device"?',
        type: 'mcq',
        options: ['A security exploit in `strcpy`.', 'A way to implement coroutines in C.', 'An optimized loop unrolling technique using a `switch` statement inside a `do-while` loop.', 'A preprocessor macro for debugging.'],
        correctAnswer: 'An optimized loop unrolling technique using a `switch` statement inside a `do-while` loop.',
        explanation: 'Duff\'s Device is a famous (and often confusing) C construct that unrolls a loop for copying data by interleaving a `switch` with a `do-while` to handle the remainder of items.',
      },
      {
        id: 40,
        text: 'What are "integer promotions" in C?',
        type: 'mcq',
        options: ['Giving an integer a higher priority in a calculation.', 'The process where integer types smaller than `int` or `unsigned int` are converted to `int` or `unsigned int` before an operation.', 'A way to guarantee an integer does not overflow.', 'Casting an integer to a `long`.'],
        correctAnswer: 'The process where integer types smaller than `int` or `unsigned int` are converted to `int` or `unsigned int` before an operation.',
        explanation: 'For example, if you add two `char` variables, they are first promoted to `int` before the addition is performed. This can sometimes lead to surprising results if signs are not handled carefully.',
      },
      {
        id: 41,
        text: 'Why is using the `gets()` function dangerous?',
        type: 'mcq',
        options: ['It is very slow.', 'It is not supported by modern compilers.', 'It performs no bounds checking, leading to buffer overflows.', 'It cannot read spaces.'],
        correctAnswer: 'It performs no bounds checking, leading to buffer overflows.',
        explanation: '`gets()` will read from standard input until a newline is found, regardless of the size of the destination buffer. This makes it trivial to write past the end of the buffer, which is a major security vulnerability. It was removed from the C11 standard.',
      },
      {
        id: 42,
        text: 'What is the behavior of `fflush(stdin)`?',
        type: 'mcq',
        options: ['It is a standard way to clear the input buffer.', 'It flushes the standard output buffer.', 'Its behavior is undefined by the C standard.', 'It closes the standard input stream.'],
        correctAnswer: 'Its behavior is undefined by the C standard.',
        explanation: 'The C standard only defines the behavior of `fflush` for output or update streams. Applying it to an input stream like `stdin` results in undefined behavior, even though some compilers may provide a specific implementation for it.',
      },
      {
        id: 43,
        text: 'The `qsort()` function requires a pointer to a comparison function. What should this function return if the first argument is "less than" the second?',
        type: 'mcq',
        options: ['A negative integer.', 'Zero.', 'A positive integer.', '`true`'],
        correctAnswer: 'A negative integer.',
        explanation: 'The comparison function `int cmp(const void *a, const void *b)` should return < 0 if a < b, 0 if a == b, and > 0 if a > b.',
      },
      {
        id: 44,
        text: 'What is "type-punning" through a union?',
        type: 'mcq',
        options: ['Casting a union to a different struct type.', 'A joke about C types.', 'Storing data as one type in a union and reading it as another type, a behavior explicitly allowed since C99.', 'It is always undefined behavior.'],
        correctAnswer: 'Storing data as one type in a union and reading it as another type, a behavior explicitly allowed since C99.',
        explanation: 'While type-punning via pointer casting violates strict aliasing, using a union for this purpose is a well-defined and common technique for inspecting the byte representation of data.',
      },
      {
        id: 45,
        text: 'What does the `#line` preprocessor directive do?',
        type: 'mcq',
        options: ['Draws a line in the console output.', 'Changes the compiler\'s reported line number and filename, often used by code generators.', 'Includes a line from another file.', 'A directive for linking.'],
        correctAnswer: 'Changes the compiler\'s reported line number and filename, often used by code generators.',
        explanation: '`#line 100 "new_file.c"` would make the compiler believe the next line is line 100 of "new_file.c", which is useful for error reporting in generated code.',
      },
      {
        id: 46,
        text: 'In C11, `_Static_assert` provides what functionality?',
        type: 'mcq',
        options: ['An assertion that is checked at runtime.', 'An assertion that is checked at compile-time.', 'A special type of function.', 'A way to halt the program.'],
        correctAnswer: 'An assertion that is checked at compile-time.',
        explanation: 'Static assertions allow you to check conditions at compile time, such as the size of a data type. If the condition is false, compilation fails with an error message. Syntax: `_Static_assert(condition, "message");`',
      },
      {
        id: 47,
        text: 'Which C library provides support for complex number arithmetic?',
        type: 'mcq',
        options: ['`<math.h>`', '`<complex.h>`', '`<stdlib.h>`', '`<fenv.h>`'],
        correctAnswer: '`<complex.h>`',
        explanation: 'The `<complex.h>` header, introduced in C99, defines the `complex` macro and provides functions for working with complex numbers.',
      },
      {
        id: 48,
        text: 'What is the purpose of the floating-point environment library `<fenv.h>`?',
        type: 'mcq',
        options: ['To provide faster floating-point functions.', 'To allow control over rounding modes and access to floating-point status flags (e.g., overflow, divide-by-zero).', 'To define floating-point constants.', 'To interface with graphics hardware.'],
        correctAnswer: 'To allow control over rounding modes and access to floating-point status flags (e.g., overflow, divide-by-zero).',
        explanation: '`<fenv.h>` provides a way to interact with and control the floating-point processing environment, which is crucial for high-precision numerical analysis.',
      },
      {
        id: 49,
        text: 'Can a function in C return an array?',
        type: 'mcq',
        options: ['Yes, directly like `int[] my_func()`.', 'No, functions cannot return arrays directly.', 'Yes, but you can wrap the array in a struct and return the struct.', 'Both B and C are correct.'],
        correctAnswer: 'Both B and C are correct.',
        explanation: 'C functions cannot directly return an array type. The common workarounds are to return a pointer to the array (which must be static or dynamically allocated) or to wrap the array inside a struct and return the struct by value.',
      },
      {
        id: 50,
        text: 'What is the `const` qualifier on an array parameter in a function definition for, e.g., `void func(const int arr[])`?',
        type: 'mcq',
        options: ['It has no effect since arrays are passed by pointer.', 'It prevents the function from modifying the elements of the original array.', 'It makes the pointer `arr` itself constant.', 'It is a syntax error.'],
        correctAnswer: 'It prevents the function from modifying the elements of the original array.',
        explanation: 'Even though `arr` is treated as a pointer (`int *`), the `const` applies to the elements. It signals to the compiler and to other programmers that the function will not (and cannot) change the contents of the array passed to it.',
      },
      {
        id: 51,
        text: 'What is the difference between `#include <file.h>` and `#include "file.h"`?',
        type: 'mcq',
        options: ['There is no difference.', '<> is for standard library headers, "" is for user-defined headers.', '<> searches a standard list of system directories first, "" searches the current directory first.', '"" is for C++, <> is for C.'],
        correctAnswer: '<> searches a standard list of system directories first, "" searches the current directory first.',
        explanation: 'While <> is conventionally used for system headers and "" for local headers, the actual defined behavior is about the search path. "" typically searches where the source file is located first, then falls back to the system path. <> searches only the system path.',
      },
      {
        id: 52,
        text: 'What is the value of `x` after the following code? `int x = -5; x = x >> 1;`',
        type: 'mcq',
        options: ['-2', '-3', '-2.5', 'Implementation-defined'],
        correctAnswer: 'Implementation-defined',
        explanation: 'Right-shifting a negative number is implementation-defined in C. Most systems perform an arithmetic shift (preserving the sign bit), which would result in -3, but a logical shift (filling with zero) is also possible.',
      },
      {
        id: 53,
        text: 'What will this code likely print? `printf("%d\\n", (int)sizeof(void));`',
        type: 'mcq',
        options: ['0', '1', 'It will not compile.', 'Implementation-defined'],
        correctAnswer: 'It will not compile.',
        explanation: 'The `sizeof` operator cannot be applied to an incomplete type like `void`. It must be used on a complete object type or a variable.',
      },
      {
        id: 54,
        text: 'What is a "trigraph sequence"?',
        type: 'mcq',
        options: ['A three-character sequence for logical operators.', 'A sequence of three characters that the compiler replaces with a single character, e.g., `??=` for `#`.', 'A comment style.', 'A debugging feature.'],
        correctAnswer: 'A sequence of three characters that the compiler replaces with a single character, e.g., `??=` for `#`.',
        explanation: 'Trigraphs were created for character sets that did not have all of C\'s punctuation characters. For example, `??(` becomes `[`. They are mostly a historical artifact and were removed in C17.',
      },
      {
        id: 55,
        text: 'A function declared with `__attribute__((constructor))` in GCC/Clang will...',
        type: 'mcq',
        options: ['be called automatically before `main()`.', 'be treated as a class constructor.', 'only be callable by `main()`.', 'not be optimized.'],
        correctAnswer: 'be called automatically before `main()`.',
        explanation: 'This is a compiler-specific extension that allows you to register functions to run before the program\'s main execution begins, useful for setting up library state.',
      },
      {
        id: 56,
        text: 'What does "short-circuit evaluation" refer to in logical operators?',
        type: 'mcq',
        options: ['A hardware failure.', 'The second operand of `&&` or `||` is not evaluated if the result can be determined from the first operand.', 'A faster way to perform arithmetic.', 'A preprocessor optimization.'],
        correctAnswer: 'The second operand of `&&` or `||` is not evaluated if the result can be determined from the first operand.',
        explanation: 'For `a && b`, if `a` is false, `b` is not evaluated. For `a || b`, if `a` is true, `b` is not evaluated. This is a guaranteed behavior in C.',
      },
      {
        id: 57,
        text: 'What is the purpose of the `offsetof` macro in `<stddef.h>`?',
        type: 'mcq',
        options: ['To get the memory offset of a file.', 'To calculate the offset in bytes of a member from the start of its parent struct.', 'To get the address of a variable.', 'To align a struct in memory.'],
        correctAnswer: 'To calculate the offset in bytes of a member from the start of its parent struct.',
        explanation: '`offsetof(struct_type, member_name)` is a compile-time constant expression that is useful in various low-level programming and serialization tasks.',
      },
      {
        id: 58,
        text: 'In C11, what does the `_Atomic` type qualifier do?',
        type: 'mcq',
        options: ['It signifies an extremely small variable.', 'It makes the variable volatile.', 'It ensures that operations on the variable are atomic (indivisible) in a multi-threaded context.', 'It creates a constant variable.'],
        correctAnswer: 'It ensures that operations on the variable are atomic (indivisible) in a multi-threaded context.',
        explanation: '`_Atomic` types and the functions in `<stdatomic.h>` are used for lock-free programming, ensuring that reads and writes to a variable are not torn or interrupted by another thread.',
      },
      {
        id: 59,
        text: 'What is the "Thundering Herd" problem in the context of C signals or threads?',
        type: 'mcq',
        options: ['A bug in the C standard library.', 'When a large number of sleeping threads or processes are awakened simultaneously for an event, causing massive contention.', 'A security vulnerability.', 'A type of memory leak.'],
        correctAnswer: 'When a large number of sleeping threads or processes are awakened simultaneously for an event, causing massive contention.',
        explanation: 'This occurs when multiple processes or threads are waiting on a single resource. When the resource becomes available, all are woken up, but only one can proceed, leading to wasted CPU cycles.',
      },
      {
        id: 60,
        text: 'What is `long double` in C?',
        type: 'mcq',
        options: ['A synonym for `double`.', 'A syntax error.', 'A floating-point type that is at least as precise as `double`.', 'An integer type for very large numbers.'],
        correctAnswer: 'A floating-point type that is at least as precise as `double`.',
        explanation: '`long double` is a distinct floating-point type intended to provide higher precision than `double`. Its actual size (e.g., 80-bit, 128-bit) is implementation-defined.',
      },
      {
        id: 61,
        text: 'Can a C `switch` statement have duplicate `case` labels?',
        type: 'mcq',
        options: ['Yes, the first one that matches is used.', 'Yes, but it produces a warning.', 'No, it is a compile-time error.', 'Only if they are in different code blocks.'],
        correctAnswer: 'No, it is a compile-time error.',
        explanation: 'All `case` labels within a single `switch` statement must be unique constant integral expressions.',
      },
      {
        id: 62,
        text: 'What is "fall-through" in a `switch` statement?',
        type: 'mcq',
        options: ['A type of error.', 'When execution continues from a matching `case` label into the next `case` because there is no `break` statement.', 'The behavior of the `default` case.', 'A performance optimization.'],
        correctAnswer: 'When execution continues from a matching `case` label into the next `case` because there is no `break` statement.',
        explanation: 'The `break` statement is required to exit a switch. Without it, control "falls through" to the statements of the subsequent case, which can be a source of bugs or a deliberate control flow mechanism.',
      },
      {
        id: 63,
        text: 'In C, is a string literal (`"hello"`) modifiable?',
        type: 'mcq',
        options: ['Yes, you can change its characters.', 'Only if it is assigned to a `char` array.', 'Attempting to modify it results in undefined behavior.', 'Yes, but only the first character.'],
        correctAnswer: 'Attempting to modify it results in undefined behavior.',
        explanation: 'String literals may be stored in read-only memory. The type of a string literal is `char[]`, but any attempt to modify its contents results in UB. `char *s = "hello"; s[0] = \'H\';` is a common error.',
      },
      {
        id: 64,
        text: 'What does the `setvbuf` function do?',
        type: 'mcq',
        options: ['Sets the value of a buffer.', 'Changes the buffering mode (e.g., unbuffered, line-buffered, fully buffered) for a file stream.', 'Compares two buffers.', 'Frees a buffer.'],
        correctAnswer: 'Changes the buffering mode (e.g., unbuffered, line-buffered, fully buffered) for a file stream.',
        explanation: '`setvbuf` gives you finer control over the buffering strategy for a stream than `setbuf`. For example, you can make `stdout` line-buffered.',
      },
      {
        id: 65,
        text: 'What is the difference between `errno` and `perror`?',
        type: 'mcq',
        options: ['`errno` is a function, `perror` is a variable.', '`errno` is a global integer variable set by system calls on error; `perror` is a function that prints a user message and the string corresponding to the current `errno` value.', 'They are synonyms.', '`perror` is for pointers, `errno` for integers.'],
        correctAnswer: '`errno` is a global integer variable set by system calls on error; `perror` is a function that prints a user message and the string corresponding to the current `errno` value.',
        explanation: 'After a library function like `fopen` fails, you check if it returned an error value (e.g., NULL), and then you can inspect `errno` to find out *why* it failed. `perror("fopen failed")` is a convenient way to print that reason.',
      },
      {
        id: 66,
        text: 'What does the `...` (ellipsis) in a function prototype signify?',
        type: 'mcq',
        options: ['The function is incomplete.', 'The function takes a variable number of arguments.', 'The function returns a pointer.', 'The function is a macro.'],
        correctAnswer: 'The function takes a variable number of arguments.',
        explanation: 'The ellipsis must appear as the last parameter. It indicates that the function is variadic, and the arguments must be accessed using the `<stdarg.h>` macros.',
      },
      {
        id: 67,
        text: 'What is the purpose of the `register` storage class specifier?',
        type: 'mcq',
        options: ['To register the variable with the operating system.', 'A hint to the compiler to store the variable in a CPU register for faster access.', 'To make a variable read-only.', 'To create a global variable.'],
        correctAnswer: 'A hint to the compiler to store the variable in a CPU register for faster access.',
        explanation: 'This is largely a historical keyword. Modern compilers are very good at optimizing and usually ignore this hint, making their own decisions about what to place in registers. You also cannot take the address of a register variable.',
      },
      {
        id: 68,
        text: 'What is a "translation unit" in C?',
        type: 'mcq',
        options: ['A function that translates languages.', 'A single source file after it has been processed by the C preprocessor.', 'The final executable program.', 'A C language keyword.'],
        correctAnswer: 'A single source file after it has been processed by the C preprocessor.',
        explanation: 'The compiler acts on translation units. It takes a .c file, expands all the #includes and macros, and the resulting single block of code is the translation unit that gets compiled into an object file.',
      },
      {
        id: 69,
        text: 'What is "linkage" in C?',
        type: 'mcq',
        options: ['How a program is connected to the internet.', 'The process of creating a linked list.', 'A property of an identifier that determines if multiple declarations of it refer to the same object or function.', 'The file format of an object file.'],
        correctAnswer: 'A property of an identifier that determines if multiple declarations of it refer to the same object or function.',
        explanation: 'C has three types of linkage: external (visible across all translation units), internal (visible only within one translation unit, e.g., static globals), and none (e.g., local variables).',
      },
      {
        id: 70,
        text: 'What happens if you `malloc` a size of 0?',
        type: 'mcq',
        options: ['It is a compile error.', 'The return value is implementation-defined; it may be NULL or a unique pointer.', 'It is guaranteed to return NULL.', 'It allocates a default size of 1 byte.'],
        correctAnswer: 'The return value is implementation-defined; it may be NULL or a unique pointer.',
        explanation: '`malloc(0)` is not guaranteed to return NULL. The implementation might return a non-NULL pointer that cannot be dereferenced but must be passed to `free` to avoid a leak.',
      },
      {
        id: 71,
        text: 'In the context of the C standard, what is an "lvalue"?',
        type: 'mcq',
        options: ['A value that is less than another value.', 'An expression that designates an object, which has a location in memory.', 'A literal value like the number 5.', 'A left-handed value.'],
        correctAnswer: 'An expression that designates an object, which has a location in memory.',
        explanation: 'An lvalue is an expression that can appear on the left-hand side of an assignment. Variables are lvalues, but constants (like `5`) and results of many expressions are not (they are rvalues).',
      },
      {
        id: 72,
        text: 'What is the purpose of `call_once` in the C11 threads library (`<threads.h>`)?',
        type: 'mcq',
        options: ['To ensure a function is called only once in its lifetime.', 'To ensure an initialization function is called by exactly one thread, exactly once.', 'A debugging function.', 'To schedule a function call for later.'],
        correctAnswer: 'To ensure an initialization function is called by exactly one thread, exactly once.',
        explanation: '`call_once` uses a `once_flag` to guarantee that even if multiple threads try to initialize a shared resource concurrently, the initialization routine is executed only a single time.',
      },
      {
        id: 73,
        text: 'Which of the following creates a thread using the C11 threads library?',
        type: 'mcq',
        options: ['`thrd_create()`', '`pthread_create()`', '`thread_start()`', '`_beginthread()`'],
        correctAnswer: '`thrd_create()`',
        explanation: '`thrd_create()` is the standard C11 function for creating a new thread. `pthread_create` is part of the POSIX threads (pthreads) library, which is a separate standard.',
      },
    ],
  },
];
